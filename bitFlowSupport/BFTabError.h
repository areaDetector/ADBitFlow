...
typedef enum _BFRC_Enum
{
	...
	BF_CAM_NOT_FOUND,					// The requested camera no found in the list of open cameras
	BF_CAM_LIST_EMPTY,					// The list of open cameras is empty.
	...
	BF_GET_CFG_LINK_WIDTH_AND_SPEED_ERR,// BFGetCfgLinkWidthAndSpeed Error
	...
	BF_REG_ADDR_ERR,					// BFRegAddr Error
	...
	BF_FLASH_MEM_IS_BUSY,				// Flash is busy when attempting a new read/write operations
	BF_FLASH_MEM_ILLEGAL_WRITE,			// Illegal flash write operation
	BF_FLASH_MEM_ILLEGAL_ERASE,			// Illegal flash erase operation
	...
	BF_CXP_CANT_SET_SPEED,				// Error setting CXP link speed
	BF_CXP_CANT_SET_REV,				// Error setting device revision
	BF_ITQ_GET_IN_ADDR_ERR,				// BFIntTimeQueueGetInAddr Error
	BF_ITQ_GET_TIME_ERR,				// BFIntTimeQueueGetTime Error
	BF_IMQ_GET_IN_ADDR_ERR,				// BFIntMetadataQueueGetInAddr Error
	BF_IMQ_GET_PACKET_ERR,				// BFIntMetadataQueueGetTime Error
	BF_NOT_ENOUGH_LINKS,				// Configured camera requies more links than are connected
	BF_BAD_CAMERA_CONNECT,				// Error with the connected camera, please check the cabling
	BF_BAD_LINK_RESET,					// Camera's Link reset register is not responding correctly
	BF_BAD_RESYNC,						// Error reseting link
	BF_BAD_FW,							// Firmware on this board is not compatible with this SDK
	BF_UPDATE_FW,						// Please update this board by running FWDownload
	BF_DMA_NO_SUPPORT,					// This DMA engine does not support this operation
	BF_REG_NOT_ON_THIS_BRD,				// This register is not on this device
	BF_CXP_MALFORMED_EVENT,				// Event packet is malformed
	BF_CXP_LINK_NOT_ALIGNED,			// Link not aligned
	BF_DO_NOTHING_ERROR,				// This is a test error message triggered by calling DoNothing

    BF_BFGTL_BAD_OPTIONS_ERR,           // The options specified are unknown or unsupported by this release.
    BF_BFGTL_CTI_ERR,                   // An unknown error occured accessing the GenTL producer.
    BF_BFGTL_BOARD_INDEX_ERR,           // Unable to determine the system index of the board handle.
    BF_BFGTL_CTI_OPEN_ERR,              // Failed to open the GenTL producer file.
    BF_BFGTL_CTI_INIT_ERR,              // Unable to initialize the GenTL producer library.
    BF_BFGTL_CTI_FUNC_MISSING,          // A required function is missing from the GenTL producer library.
    BF_BFGTL_DEV_INIT_ERR,              // Unable to initialize the GenTL TLDevice module.
    BF_BFGTL_DEV_PORT_OPEN_ERR,         // Failed to open the TLDevice port.
    BF_BFGTL_DEV_NODE_MAP_LOAD_ERR,     // An error occurred while loading the remote device node map.
    BF_BFGTL_DEV_NODE_MAP_OPEN_ERR,     // The remote device node map could not be opened.
    BF_BFGTL_NODE_INDEX_OUT_OF_RANGE,   // The specified node index exceeds the node map size.
    BF_BFGTL_NODE_ID_MISSING,           // The node map doesn't contain a node of the specified name.
    BF_BFGTL_NODE_TYPE_UNKNOWN,         // Encountered a GenApi INode of an unknown type.
    BF_BFGTL_NODE_TYPE_MISMATCH,        // Encountered a GenApi INode of an unexpected type.
    BF_BFGTL_NODE_FIELD_ERR,            // The BFGTLNodeField value is unknown, or unsupported for the operation.
    BF_BFGTL_NODE_SIZE_ERR,             // The data cannot be read because the buffer size is inadequate or incorrect.
    BF_BFGTL_NODE_STR_SIZE_WARN,        // The string could not be read in its entirety, because the input buffer is too small.
    BF_BFGTL_NODE_INQ_CODE,             // The BFGTLNodeInqProp value is unknown.
    BF_BFGTL_GC_GENERIC_ERR,            // Caught a GenICam Generic exception.
    BF_BFGTL_GC_INVALID_ARG_ERR,        // Caught a GenICam InvalidArgument exception.
    BF_BFGTL_GC_OUT_OF_RANGE_ERR,       // Caught a GenICam OutOfRange exception.
    BF_BFGTL_GC_PROPERTY_ERR,           // Caught a GenICam Property exception.
    BF_BFGTL_GC_RUNTIME_ERR,            // Caught a GenICam Runtime exception.
    BF_BFGTL_GC_LOGICAL_ERR,            // Caught a GenICam Logical exception.
    BF_BFGTL_GC_ACCESS_ERR,             // Caught a GenICam Access exception.
    BF_BFGTL_GC_TIMEOUT_ERR,            // Caught a GenICam Timeout exception.
    BF_BFGTL_GC_DYNAMIC_CAST_ERR,       // Caught a GenICam DynamicCast exception.
    BF_BFGTL_OUT_OF_MEMORY,             // Unable to perform an internal allocation.
    BF_BFGTL_UNKNOWN_EXCEPTION_ERR,     // An unknown exception was thrown internally.

    BF_LOGIO_SOURCE_ACQUIRE_ERR,        // BFLogIO was unable to acquire a source handle.
    BF_LOGIO_CONNECT_ERR,               // Failed to establish a BFLogIO source-sync connection.
    BF_LOGIO_CONNECT_NO_SYNC_WARN,      // Unable to connect BFLogIO source to sync because no sync exists.
    BF_LOGIO_MAKE_MESSAGE_ERR,          // Failed to create a BFLogIO message.
    BF_LOGIO_MESSAGE_PUSH_ERR,          // Failed to push a message from the source to sync.
    BF_LOGIO_INVALID_TYPE,              // The message type flags specified are unknown or invalid.
    BF_LOGIO_UNKNOWN_SOURCE,            // The message source specified is unknown.
