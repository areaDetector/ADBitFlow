...
#define BiTypeGn2					CISYS_TYPE_GN2  // GN2
...
#define BiCamInqPackedPixels			CiCamInqPackedPixels			// Packed Pixels
...
#define HighFrameRateMode				16384
#define FourHorizInterleavedChunks0		0x00008000
#define FourHorizInterleavedChunks1		0x00008001
#define FourHorizInterleavedChunks2		0x00008002
#define FourHorizInterleavedChunks3		0x00008003
...
typedef struct _BFBUFARY
{
	...
	BFU32			BuffersFullSemCount;		// Number of buffers queued by the BuffersFull Semaphore
	BFU32			NumBuffersMinusHolds;		// Number of buffers that are currently available
	...
	BFU32			AqSetupOptions;				// (BiAqEngJ,BiAqEngK),(IgnoreOverflows,AbortOnError,DisableAqErrorSig)
	...
	BFU32			NumErr;						// Number of ErrorWait handles
	...
	BFBOOL			UsingHighFrameRateMode;		// TRUE <=> Use high frame rate interrupt mode (Gn2 only)
	BFU64			LastIntTime;				// The last hi res time stamp from the ISR
	BFU32			CXPSrcTag;					// Source Tag of this frame (CXP only in continuous image header record mode)
	BFU32			CXPXOffset;					// XOffset of this frame (CXP only in continuous image header record mode)
	BFU32			CXPYOffset;					// XOffset of this frame (CXP only in continuous image header record mode)
	...
	PBFVOID			pUserData;					// Pointer to user defined structure

	// Pitch stuff
	BFU32			Pitch;						// Set to non-zero to use the non-default pitch

	// High frame rate polling stuff
	BFPollInfoRec	PollHandle;					

	// ROI cache to speed access to main parameters
	BFU32			BiCacheXSize;					// Cache line width 
	BFU32			BiCacheYSize;					// Cache frame height 
	BFU32			BiCachePixDepth;				// Cache pixel depth (bits)
	BFU32			BiCachePixByteDepth;			// Cache pixel depth (bytes)
	BFU32			BiCachePacked;				// Cache packed options
	BFU32			BiCacheFormat;				// Cache Tap format 
	BFU32			BiCacheTimeout;				// Cache timeout
