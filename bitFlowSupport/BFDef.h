...
// Structure used for linked list of VFGs 
typedef struct _VFGList
{
	struct _VFGList	*Next;		// Point to the next VFGList Entry;
	BFU32		DeviceID;		// Device ID of VFG
	BFU32		EntryIndex;		// Index of entry
	BFENTRY		Entry;			// BFENTRY structure used for opening board
	BFBOOL		IsSynthetic;	// Set to true if the VFG has a synthetic camera file attached
	BFBOOL		IsCXP;			// Set to true if the VFG is CXP
	BFBOOL		IsValid;		// Set to true if the VFG can be opened
	BFU32		MasterLink;		// For CXP cameras, the link connected to the camera's master link
	BFU32		MyLinks[4];		// List of associated links
	BFU32		MyCamerasLinks[4];	// List of the camera's links that this VFG is using
	BFU32		NumLinks;		// Total number of links associated with this VFG

} VFGListRec, *VFGListPtr;

// Structure use to describe CXP cameras 
typedef struct _LinkCam
{
	BFU32	VFGNum;			// VFG number that should be used to control/acquire from this camera
	BFU32	NumLinks;		// Number of (frame grabber) links this camera uses
	BFU32	MasterLink;		// The frame grabber link number that is this cameras master link
	BFU32	MyLinks[4];		// List of frame grabber links this VFG is using
	BFU32	MyCamerasLinks[4];	// List of the camera's links that this VFG is using
} LinkCamRec, *LinkCamPtr;
...
typedef struct _BFSIGNAL 
{
	...
	BFU32	MetadataAddr;
	BFU32	MetadataCount;
	BFU32	MetadataPacketSize;
	BFU32	MetadataQueueSize;
} BFSIGNAL, *PBFSIGNAL;

// Timing info
typedef struct _BFSignalTimeInfoRec
{
	BFU32	Metadata[MAX_PACKET];		// use when collecting interrupt metadata
	BFU64	TimeStamp;
...
{
	PBFCNF OpenCam;					// Pointer to camera structure
	BFU32  Index;					// Index in linked list of open cameras
	BFCHAR ConfigMode[MAX_STRING];	// Name of mode requested (gen2 only)
	BFCHAR FileName[MAX_STRING];	// Name of the camera file
...
#define BFDISP_FORMAT_GEN2_30BIT	0x0010
...
typedef enum _BFFormats
{
	...
	BFFormat4XE_1Y,						// Four taps, four segments, two right mirrored
	...
	BFFormat1X2_2Y,						// Four taps, top/bottom, bottom, odd/even pixel
	...
	BFFormat1X5_1Y,						// Five taps, five way interleaved
	BFFormat1X5_2Y,						// Five taps, five way interleaved, top/bottom, bottom nomral
	BFFormat1X5_2YE,					// Five taps, five way interleaved, top/bottom, bottom inverted
	BFFormat1X5_2YM,					// Five taps, five way interleaved, top/bottom, top inverted
	BFFormat1X6_1Y,						// Six taps, six way interleaved
	BFFormat6X_1Y,						// Six taps, six segments
	...
	BFFormat5X2_1Y,						// Ten taps, five segments, odd/even pixels
	BFFormat5X2_2YE,					// Ten taps, five segments, odd/even pixels, top/bottom, bottom inverted
	BFFormat5X2_2YM,					// Ten taps, five segments, odd/even pixels, top/bottom, top inverted
	...
	BFFormatCustom,						// Manual tap programming
...
{
	BFSyncModeDefault = 0,			// Normal sync mode
	BFSyncModeLinkResetWO = 1,		// Special sync mode where Link Reset is WO
	BFSyncModeNeedsPreReset = 2,	// Special mode were camera needs Channel Reset before sync
...
typedef enum _TimingSequencers
{
	TSMain,
	TSAux,
}TimingSequencers;

// structure used to pass variables to PollSignalHandler

typedef struct _BFPollInfo
{
	Bd		Board;				// Handle to the board
	BFU32	Interrupt;			// Interrupt type
	BFU32	PollRegisterID;		// Register to poll
	BFU32	PollPause;			// How long to pause between polls (microseconds)
	BFU32	RegisterMax;		// Maximum value of register (2^N where N is number of bits in register)
	BFBOOL	KeepWorking;		// A boolean, initial value should be true, set to false to kill thread
	HANDLE	PollThread;			// Handle to the polling thread
}BFPollInfoRec, *BFPollInfoPtr;

// use to determine which BFML events should be executed
typedef enum _BFMLEvents
{
	BFCamBefore,
	BFCamAfter,
} BFMLEvents;

// maximum size of event packet payload
#define MAX_EVENT_PAYLOAD 256

// CXP Event packets
typedef struct _EventPacket
{
	BFU32 MasterHostConnectionID;	
	BFU32 PacketTag;
	BFU32 NumWords;
	BFU32 MessageData[MAX_EVENT_PAYLOAD];
} EventPacketRec, *EventPacketPtr;

typedef struct _EventMessage
{
	BFU32 Size;			// Event message size in bytes
	BFU32 Namespace;	// event name space (0-GenICam, 1-CXP, 2-Device, 3-Reserved
	BFU32 EventID;		// Event ID
	BFU64 TimeStamp;	// Timestamp
	PBFU32 Data;		// Event data
} EventMessageRec, *EventMessagePtr;

typedef struct _CXPHeartbeat
{
	BFU64 PacketDeviceTime;
	BFU64 PacketHostTime;
	BFU32 PacketConnectionID;
} CXPHearbeatRec, *CXPHeartbeatPtr;
