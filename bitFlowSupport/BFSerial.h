...
typedef struct _SerRefStruc
{
	...
	BFU32				OpenCount;
} SerRefStruc, *SerRefStrucPtr;

/* Shared Serial Parameter Structure */
#define MAX_OPENS_PER_PORT 5
typedef struct _SharedSerStruc
{
	BFU32		OpenCount;
	PBFVOID		pSerRef[MAX_OPENS_PER_PORT];
	BFU32		ProcID[MAX_OPENS_PER_PORT];
} SharedSerStruc, *SharedSerStrucPtr;
...
BFSERIALEXPORT int BFSERIALCC
clBFSerNumtFromBoardHandle(Bd hBoard, PBFU32 pSerNum);
