...
// Note: as of 6.40, it is no longer importante to keep these in any kind of order, they just need to be unique
...
//#define KCXPDriverOnly	6 // removed, note that these have to be in numeric order for all psued drivers regsitered
#define Gn2DriverOnly		7
#define NDifDriverOnly		8
...
#define BFK_LINK_WIDTH		"CfgLinkWidth"
#define BFK_LINK_SPEED		"CfgLinkSpeed"
...
#define BFK_DO_BLOCK_WRITE	"DoBlockWrite"
...
#define BFK_DEFAULT_CAM_CTN		"DefaultCtnFile"
#define BFK_DEFAULT_CAM_NDIF	"DefaultNDifFile"
#define BFK_DEFAULT_CAM_AXN		"DefaultAxnFile"
#define BFK_DEFAULT_CAM_AON		"DefaultAonFile"
#define BFK_DEFAULT_CAM_CLX		"DefaultClxFile"
...
typedef enum _DrvStates
{
	...
	DrvSetupPseudoGn2,								// Perform Karbon GN2 pseudo device setup.
	DrvSetupPseudoNeonDif,							// Perform NeonDif pseudo device setup.
} DrvStates;
...
// maximum number of links per board
#define MAX_LINKS_PER_BOARD	4	// NEEDSWORK: may need bump up this number

// maximum number of Virtual Deviced per board
#define MAX_VD				3

// Min and max firmware compatiblity revisions
// These are the major revisions of the firmware that this driver is compatible with
// Firmware revisions outside of these limits are not complatible with this driver
// and the board's firmware must be updated to this SDK's firmware 
#define BF_DRIVER_CXP_CMPTBL_REV_MIN 1
#define BF_DRIVER_CXP_CMPTBL_REV_MAX 1
...
// The following section is for user level only definitions.
    ...
    typedef struct _BFInfoUsr
    {
	    ...
	    HANDLE				hLinkMutex[MAX_LINKS_PER_BOARD];	// Handle of mutexes used to serialize CXP communication
		...
		PBFS32				IntEnableCount;				// Count number of signals created, disable interrupt when last signal is freed.
		HANDLE				hIntCountMap;				// handle to shared memory used to store IntEnableCount
		BFBOOL				PollingInts[BFIntCount];	// set to true for ints that are polling
		...
		POPENCAMLIST		pOpenCamList;				// Open camera files that are attached to the board via SysReg
		POPENCAMLIST		pUserCamList;				// Open camera files that user opened
		...
		BFU32				ControlPacketResponseTimeout;	// Control packet response timeout (can be overridden in registry)
		BFU32				LinkList[MAX_LINKS_PER_BOARD];	// The list of links for this VFG in order that we should access them (should match camera)
		BFU32				CXPRevision;					// Revision CXP inteface is running
		BFU32				CXPCommandTag;					// Command packet tag
		BFU32				CXPCommandTagOut;				// Last Command packet tag sent
		BFU32				CXPCommandTagIn;				// Last Command packet tag received

		// BFML stuff (Gen2 only)
		BFCHAR				CurrentConfigMode[MAX_STRING];		// Name of current mode
		BFCHAR				RequestedConfigMode[MAX_STRING];	// Name of mode requested
		BFCHAR				LastOpenedConfigMode[MAX_STRING];	// Name of last mode that has been opened

		// User ROI override settings (Gn2 only)
		BFU32				UserXSize;					// User line width override
		BFU32				UserXOffset;				// User X Offset override
		BFU32				UserYSize;					// User frame height override
		BFU32				UserYOffset;				// User Y Offset override
		BFU32				UserPixDepth;				// User pixel depth override
		BFU32				UserAqTimeout;				// User AQ timtout override
		BFU32				UserFormat;					// User Tap format override
		BFU32				UserCamLineWidth;			// User camera line width override

		// ROI cache (saves time getting common values) (Gn2 only)
		BFU32				CacheXSize;					// Cache line width 
		BFU32				CacheYSize;					// Cache frame height 
		BFU32				CachePixDepth;				// Cache pixel depth (bits)
		BFU32				CachePixByteDepth;			// Cache pixel depth (bytes)
		BFU32				CachePacked;				// Cache packed options
		BFU32				CacheFormat;				// Cache Tap format 
		BFU32				CacheTimeout;				// Cache timeout

		// Cache Valid flags for ROI cache variables above
		BFBOOL				CacheValidXSize;			// Cache valid for line width 
		BFBOOL				CacheValidYSize;			// Cache valid for frame height 
		BFBOOL				CacheValidPixDepth;			// Cache valid for pixel depth (bits)
		BFBOOL				CacheValidPixByteDepth;		// Cache valid for pixel depth (bytes)
		BFBOOL				CacheValidPacked;			// Cache valid for packed options
		BFBOOL				CacheValidFormat;			// Cache valid for Tap format 
		BFBOOL				CacheValidTimeout;			// Cache valid timeout

		// Gen 2 VFG stuff
		BFU32				VFGNum;						// VFGNum on the board
		BFU32				CXPSyncMode;				// CXP sync mode
		BFU32				CXPLinkAlignTimeout;		// Timeout for links to align

		// custom tap programming (Axion only)
		BFS32				Start[10];					// Start arrray, size = MAX_TAPS
		BFU32				Step[10];					// Step array, size = MAX_TAPS
		BFU32				TapCount;					// number of taps

		// power control
		BFU32				OldPowerSetting;			// Holds the power setting before being optomized for DMA

		// For cameras that can be auto configured, the camera's infor is stored in these
		BFU32				AutoXSize;					// Width in pixels
		BFBOOL				UsingAutoXSize;				// set to true of the system is using the XSize from the camera
		BFU32				AutoYSize;					// Height in lines
		BFBOOL				UsingAutoYSize;				// set to true of the system is using the YSize from the camera
		BFU32				AutoMode;					// Aquisition mode
		BFU32				AutoStartAddress;			// Address of the start register
		BFU32				AutoStopAddress;			// Address of the stop register
		BFU32				AutoBitDepth;				// Bit depth
		BFBOOL				UsingAutoBitDepth;			// set to true of the system is using the BitDepth from the camera
		BFU32				AutoTapGeometry;			// Tap format
		BFU32				AutoNumLinks;				// Number co camera CXP links

		// new interrupt technology
		BFU32				SerBufIndxUser;
...
// The following section is for Kernel only definitions.
	...
	typedef struct _PQTabHeadRec
	{
		...
		BFBOOL			PhysicalMem;					// Set to true if destination is physical buffer
	} PQTabHeadRec;
	...
	// This structure is used to cached an individual PCI device PCI space
	// This is a link list of "things" in a devices PCI space
    typedef struct _PCIThingCache	*PCIThingCachePtr; 
    typedef struct _PCIThingCache
	{
		BFU32				Offset;
		BFU32				Size;
		PBFVOID				pThing;
		PCIThingCachePtr	Next;

	} PCIThingCacheRec;


	// This structure is used to cache the PCI device information for the current system
	// This is a link list of devices, each device has it's own link list of "things"

    typedef struct _PCIDeviceCache	*PCIDeviceCachePtr; 
    typedef struct _PCIDeviceCache
	{
		BFU32				Bus;
		BFU32				Slot;
		BFU32				Function;
		PCIThingCachePtr	pDevice;
		PCIDeviceCachePtr	Next;
	...
	typedef struct _BFDriverInfoKrn
	{
		...
		PCIDeviceCachePtr	DeviceCache;				// Cache of this system PCI Devices and their PCI Device space
		...
		BFBOOL				DownloadInProgress;			// Set to true when board is being downloaded

		BFBOOL				NoBoardAccessInISR;			// Set to true to prevent all access to the board in ISR
		...
		PBFCHAR				Gn2Root;					// Registry device path to Gen 2.
		...
		BFBOOL				Gn2DoneCleaning;			// Set True after Gen 2 device registry cleaning.
		BFU32				Gn2NumBoards;				// Number of Gen 2 boards.
		...
		BFBOOL				Gn2DoneFlags;				// Set True after Gn2 registry flags are loaded.
		BFBOOL				Gn2Test;					// Set True to enable testing of new Gn2 functions.
		BFBOOL				Gn2Stop00All;				// Set True to enable exit after driver builds Gn2Info.
		BFBOOL				Gn2MatchDeviceId;			// Set True to enable device id matching during insertion.
		BFBOOL				Gn2MatchSubVendorId;		// Set True to enable subvendor id matching during insertion.
		BFBOOL				Gn2MatchSubSystemId;		// Set True to enable subsystem id matching during insertion.
		BFBOOL				Gn2MatchRevision;			// Set True to enable revision matching during insertion.
		BFBOOL				Gn2MatchInfoHi;				// Set True to enable InfoHi matching during insertion.
		BFBOOL				Gn2MatchInfoLo;				// Set True to enable InfoLo matching during insertion.
		BFU32				Gn2MatchInfoHiMask;			// Mask used when matching InfoHi
		BFU32				Gn2MatchInfoLoMask;			// Mask used when matching InfoLo
    ...
    typedef struct _BFInfoKrn 
    {
		...
		BFBOOL				VirtualDevice;				// Set to true if this is a virtual device on a single function real device
		BFU32				FlashEraseWordBlockSize;	// This families flash block size (used during erase)
		BFU32				FlashWriteWordBlockSize;	// This families flash block size (used during write)
		PBFU32				FlashCurrentOperation;		// The current flash operations (0 - getting read, 1 - erase, 2 - write)
		PBFU32				FlashNumWordsToWritePtr;	// Number of words that must be written to the flash (used for user feedback)
		PBFU32				FlashCurrentWordPtr;		// Current word being written to the flash (used for user feedback)


		// Handles to VD's on this frame grabber
		Bd					BoardVD[MAX_VD];			// The handle to this board's Virtual Devices
		...
		BFBOOL BoardReturnedFromHibernate;					// Set to True if the board needs to be re-downloaded after hybernate
		...
		BFU32				IntNewCount[BFIntCount];	// Number of new board interrupts since last emitted interrupt
		...
		BFBOOL				HardwareTimeStamp;			// True if board has hardware time Stamp (Gen 2 only)
		BFU32				TimestampFIFODepth;			// The true depth of the hardware timestamp FIFO (register is bigger)

		// interrupt metadata
		BFU32				MetadataSize[BFIntCount];			// Size of metadata package for each interrupt in words, 0 means no metadata
		BFBOOL				MetadataCollect[BFIntCount];		// Set to true to enable collection of metadata for each interrupt
		PBFU32				MetadataPtrQueue[BFIntCount];		// pointer to the metadata queue for each interrupt 
		BFU32				MetadataPtrQueueAddr[BFIntCount];	// Current queue address for each interrupt
		BFU32				MetadataQueueSize[BFIntCount];		// size of each interrupts metadata queue
		...
		BFU32				LastBrdOpened;				// Set to the board number of the last VFG to be opened.
		...
		// Gen2 stuff
		BFU32				VFGNum;						// used to determin the VFG number on Gen2 devices
