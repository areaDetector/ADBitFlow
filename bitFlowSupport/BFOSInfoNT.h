...
#define __BFOSINFONT__
	...
	typedef struct _OSDriverInfoKrn
	{
		...
		BFU32				NumHandlesOpen;				// Total number of user level applications with an open handle (across all board)
	} OSDriverInfoKrn;
...
//	    KMUTEX				FHMutex;					// Mutex used to serialize File Handle Create/Delete operations
		...
		BFU32				InClose;					// Set to 1 when In Close has Mutex
		BFU32				InCreate;					// Set to 1 when In Create has Mutex
		BFU32				InDeviceControl;			// Set to 1 when In DeviceControl has Mutex
		BFU32				InCleanUp;					// Set to 1 when In CleanUp has Mutex
